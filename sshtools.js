...
SSHTools.prototype.reconnect = function() {
    ...
    self._sshLog("手动重连：已清空所有积压SSH任务");

    // 2. 空命令拉起连接，仅做链路检测，执行结果无业务意义
    self.exec("echo reconnect_ok", {}, (err) => {
        if (err) {
            self._sshLog("手动重连触发失败: " + err.message);
        } else {
            self._sshLog("手动重连链路重建完成，旧任务已全部丢弃，请重新下发指令");
        }
    });
};
